...
var Acl = function (backend, logger, options){
  ...
  if (!options && typeof logger === 'object' && !logger.debug) {
    options = logger;
    logger = undefined;
  }
  ...
  if (options.userBackend) {
    this.userBackend = options.userBackend;
  } else {
    this.userBackend = backend;
  }
  var userBackend = this.userBackend;
  ...
  userBackend.endAsync = bluebird.promisify(userBackend.end);
  userBackend.getAsync = bluebird.promisify(userBackend.get);
  userBackend.cleanAsync = bluebird.promisify(userBackend.clean);
  userBackend.unionAsync = bluebird.promisify(userBackend.union);
  if (userBackend.unions) {
    userBackend.unionsAsync = bluebird.promisify(userBackend.unions);
  }
};
...
Acl.prototype.addUserRoles = function(userId, roles, cb){
  ...
  var transaction = this.userBackend.begin();
  this.userBackend.add(transaction, this.options.buckets.meta, 'users', userId);
  this.userBackend.add(transaction, this.options.buckets.users, userId, roles);

  if (Array.isArray(roles)) {
    var _this = this;

      roles.forEach(function(role) {
        _this.userBackend.add(transaction, _this.options.buckets.roles, role, userId);
      });
  }
  else {
      this.userBackend.add(transaction, this.options.buckets.roles, roles, userId);
  }

  return this.userBackend.endAsync(transaction).nodeify(cb);
};
...
Acl.prototype.removeUserRoles = function(userId, roles, cb){
  ...
  var transaction = this.userBackend.begin();
  this.userBackend.remove(transaction, this.options.buckets.users, userId, roles);

  if (Array.isArray(roles)) {
    var _this = this;

    roles.forEach(function(role) {
      _this.userBackend.remove(transaction, _this.options.buckets.roles, role, userId);
    });
  }
  else {
    this.userBackend.remove(transaction, this.options.buckets.roles, roles, userId);
  }
...
Acl.prototype.userRoles = function(userId, cb){
  return this.userBackend.getAsync(this.options.buckets.users, userId).nodeify(cb);
};

/**
  userAllRoles( userId, function(err, roles) )
  ...
  @return {Promise} Promise resolved with an array of user roles
*/
Acl.prototype.userAllRoles = function(userId, cb){
  var self = this;
  return this.userBackend.getAsync(this.options.buckets.users, userId).then(function (roles) {
    return self._allRoles(roles);
  }).nodeify(cb);
};
...
Acl.prototype.roleUsers = function(roleName, cb){
  return this.userBackend.getAsync(this.options.buckets.roles, roleName).nodeify(cb);
};
...
Acl.prototype.addRoleParents = function(role, parents, cb){
  ...
  var transaction = this.userBackend.begin();
  this.userBackend.add(transaction, this.options.buckets.meta, 'roles', role);
  this.userBackend.add(transaction, this.options.buckets.parents, role, parents);
  return this.userBackend.endAsync(transaction).nodeify(cb);
};
...
Acl.prototype.removeRoleParents = function(role, parents, cb){
  ...
  var transaction = this.userBackend.begin();
  if (parents) {
    this.userBackend.remove(transaction, this.options.buckets.parents, role, parents);
  } else {
    this.userBackend.del(transaction, this.options.buckets.parents, role);
  }
  return this.userBackend.endAsync(transaction).nodeify(cb);
};
...
Acl.prototype.removeRole = function(role, cb){
  ...
  return this.backend.getAsync(this.options.buckets.resources, role).then(function(resources){
    ...
    var userTransaction = _this.userBackend.begin();
    ...
    _this.userBackend.del(userTransaction, _this.options.buckets.parents, role);
    _this.userBackend.del(userTransaction, _this.options.buckets.roles, role)
    _this.userBackend.remove(userTransaction, _this.options.buckets.meta, 'roles', role);

    // `users` collection keeps the removed role
    // because we don't know what users have `role` assigned.
    _this.userBackend.endAsync(userTransaction);
...
Acl.prototype.removeResource = function(resource, cb){
  ...
  var _this = this;
  return this.userBackend.getAsync(this.options.buckets.meta, 'roles').then(function(roles){
    var transaction = _this.backend.begin();
...
Acl.prototype.allow = function(roles, resources, permissions, cb){
  ...
  if((arguments.length == 1) || ((arguments.length===2)&&_.isObject(roles)&&_.isFunction(resources))){
    ...
    _this.userBackend.add(userTransaction, _this.options.buckets.meta, 'roles', roles);
...
Acl.prototype._rolesParents = function(roles){
  return this.userBackend.unionAsync(this.options.buckets.parents, roles);
};
  ...
  return this.backend.unionAsync(allowsBucket(resource), roles).then(function(resourcePermissions){
    ...
    if (resourcePermissions.indexOf('*') !== -1){
      ...
      if(permissions.length === 0){
        ...
        return _this.userBackend.unionAsync(_this.options.buckets.parents, roles).then(function(parents){
          if(parents && parents.length){
